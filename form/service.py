...
class ServiceAdmin:
	...
	def showList(self, query):
		...
		if (self.query.count() > 0):
			for record in self.query:
				item = "%-20s %-15s %6s %-15s %-5s" % (record.nombre, record.descripcion, str(record.puerto_desde), str(record.puerto_hasta), record.protocolos)
				self.listElement.append(item, record)
		else:
			self.listElement.append(gettext.gettext("There are no records"), 0)
		self.grid = GridForm(self.screen, gettext.gettext('Service List'), 1, 2)
	...
	def addService(self, title, text, screen, table):
		self.screen = screen
		self.title = title
		self.text = text
		#self.option = option
		...
		if rta[0] == "save":
			...
			registro=self.table(nombre=rta[1][0], descripcion=rta[1][1],
								puerto_desde=int(rta[1][2]),
								puerto_hasta=int(rta[1][3]),
								protocolos=rta[1][4])
		return rta
	
	def editService(self, title, text, screen, table, registro):
		self.screen = screen
		self.title = title
		self.text = text
		self.table = table
	...
	def deleteService(self, screen, table, registro):
		self.screen = screen
		self.table = table
		self.registro = registro
